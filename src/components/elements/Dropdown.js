...
export const Dropwdown = (props) => {
    ...
    useEffect(() => {
        if (open) {
            const optionChildren = dropdownRef.current.querySelectorAll(
                '.dropdown-options > *'
            )
            const dropdownOptionsContainer = dropdownRef.current.querySelector(
                '.dropdown-options-wrapper'
            )

            dropdownOptionsContainer.style.whiteSpace = 'nowrap'

            const maxChildWidth = Array.from(optionChildren).reduce(
                (currWidth, child) => {
                    return currWidth <= child.offsetWidth
                        ? child.offsetWidth
                        : currWidth
                },
            ...
            dropdownOptionsContainer.style.minWidth =
                Math.min(maxChildWidth, DROPDOWN_OPTIONS_CONTAINER_MAX_WIDTH) +
                DROPDOWN_OPTIONS_CONTAINER_PADDING +
                'px'
