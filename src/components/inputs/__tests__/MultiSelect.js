...
test('Show default values', () => {
    const { selectInputList, getListElement } = renderMultiSelect({value: ['1', '2']});
...
test('Call onChange event with selected value', () => {
    ...
    const { getValue, getOption } = renderMultiSelect({value: [], onChange: handleChange});
...
test('Call onRemoveTag event with clicked value', () => {
    ...
    const { getListElement } = renderMultiSelect({value: ['3', '4'], onRemoveTag});
